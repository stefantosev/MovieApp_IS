...
namespace MovieApp.Controllers
{
    public class TicketsController : Controller
    {
            ...
            if (id == null)
        {
            ...
            var shoppingCart = _context.Order.FirstOrDefault(x => x.OwnerId == userId);

            TicketInOrder ps = new TicketInOrder();

            if (ticket != null && shoppingCart != null)
            {
                ps.TicketId = ticket.Id;
                ps.OrderId = shoppingCart.Id;
            }
            return View(ps);
        }

        [HttpPost]
        public async Task<IActionResult> AddToOrderConfirmed(TicketInOrder model)
        {
            var userid = User.FindFirstValue(ClaimTypes.NameIdentifier);

            var order = _context.Order.FirstOrDefault(x => x.OwnerId == userid);

            if (order != null)
            {

                if (order.TicketInOrders == null)
                    order.TicketInOrders = new List<TicketInOrder>();
            }

            order.TicketInOrders.Add(model);
